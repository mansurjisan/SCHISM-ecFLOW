...
def put_sflux_fields(iday, date, timevector, ds, nx_grid, ny_grid, air, rad, prc, output_interval, OUTDIR):
    ...
    # Find the index for midnight of the current day
    try:
        target_time = date.replace(hour=0, minute=0, second=0, microsecond=0)
        idx = timevector.get_loc(target_time)
    except KeyError:
        # If exact match not found, find nearest time
        idx = timevector.get_indexer([target_time], method='nearest')[0]
        print(f"Warning: Using nearest time match for {target_time}")
    ...
    if air:
        with nc.Dataset(OUTDIR / f"sflux_air_1.{iday+1:04d}.nc", 'w', format='NETCDF3_CLASSIC') as dst:
            ...
            v_time.units = f'days since {date.year}-{date.month:02d}-{date.day:02d} 00:00 UTC'
            v_time.base_date = (date.year, date.month, date.day, 0)
            v_time[:] = times
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rnday', type=int, required=True)
    parser.add_argument('--hgrid', type=str, required=True)
    parser.add_argument('--outdir', type=str, required=True)
    
    args = parser.parse_args()

    startdate = datetime(args.year, args.month, args.day)
    rnday = args.rnday
    t0 = time()
    ...
    interval = 1
    local_era5_file = 'data_stream-oper.nc'
    ...
    timevector = pd.to_datetime(times)
    
    for iday in range(rnday):
        date = startdate + timedelta(days=iday)
        put_sflux_fields(iday, date, timevector, ds, nx_grid, ny_grid,
