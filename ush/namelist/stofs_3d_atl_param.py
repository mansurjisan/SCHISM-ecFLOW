...
! Global output controls
  ...
  ihfskip = 288 ! zy; Fei:576 !stack spool; every ihfskip steps will be put into 1_*, 2_*, etc...
/
...
! Main ramp option
!-----------------------------------------------------------------------
! zy: Fei commented out this line: nramp = 0 !ramp-up option (1: on; 0: off)
! nramp = 0  ! tested, MUST have!; zy rm'ed leading "!"; ramp-up option (1: on; 0: off)
  dramp = 0. ! zy- Fei original: 1. !needed if nramp=1; ramp-up period in days

! zy: Fei commented out this line: nrampbc = 0 !ramp-up flag for baroclinic force
! nrampbc = 0  ! added back "!"; zy rm'ed leading "!"; ramp-up flag for baroclinic force
  drampbc = 0. ! zy; Fei's original: 4. !not used if nrampbc=0
...
!-----------------------------------------------------------------------
  ...
  shapiro0 = 1.e2 !Shapiro filter strength, needed only if ishapiro=1; max is 0.5
...
! nramp_ss = 1 !needed if if_source=1; ramp-up flag for source/sinks
  dramp_ss = 0  ! zy; Fei original: 3 !needed if if_source=1; ramp-up period in days
...
!  see subroutine weno_diag in src/Hydro/misc_subs.F90 for details
  ...
  epsilon2 = 1.e-6  !1st coefficient for 3rd order weno smoother (larger values are more prone to numerical dispersion
...
! (Fei's original with leading "!")  nrampwind = 1 !ramp-up option for atmos. forcing
! nrampwind = 0  ! zy added back !; zy uncommented above line (Fei's original)
  drampwind = 0.  ! zy; Fei original: 2. !needed if nrampwind/=0; ramp-up period in days
...
!-----------------------------------------------------------------------
  ...
  step_nu_tr = 21600.  ! zy; Fei's original: 86400. !time step [sec] in all [MOD]_nu.nc (for inu_[MOD]=2)
...
!-----------------------------------------------------------------------
  iout_sta = 1
  nspool_sta = 2 !zy; Fei original: 12 !needed if iout_sta/=0; mod(nhot_write,nspool_sta) must=0
...
!-----------------------------------------------------------------------
  ...
  iof_hydro(11) = 1 !evaporation rate [kg/m/m/s] 
  iof_hydro(12) = 1 !precipitation rate [kg/m/m/s] 
  iof_hydro(13) = 0 !Bottom stress vector [kg/m/s^2(Pa)] 
...
!  iof_hydro(25) = 1 !z-coord 3D - this flag will be reset by code to be always on
  iof_hydro(26) = 1 !horizontal velocity vector [m/s] 
  iof_hydro(27) = 0 !vertical vel. @elem [m/s] 
  iof_hydro(28) = 0 !T @prism centers [C] 
  iof_hydro(29) = 0 !S @prism centers [PSU] 
  iof_hydro(30) = 0 !Barotropic pressure gradient force vector (m.s-2) @side centers
