...
def generate_param_nml(start_year, start_month, start_day, start_hour, rnday):
    content = f"""!parameter inputs via namelist convention.
...
! Baroclinic/barotropic option. If ibc=0 (baroclinic model), ibtp is not used.
  ...
  rnday = {rnday} !total run time in days
  dt = 10. !Time step in sec

! Grid for WWM (USE_WWM)
  msc2 = 12     !same as msc in .nml ... for consitency check between SCHISM and WWM
  mdc2 = 12     !same as mdc in .nml
...
! Global output controls
  ...
  ihfskip = 180 !stack spool; every ihfskip steps will be put into 1_*, 2_*, etc...
...
! Starting time
  ...
  start_month = {start_month} !int
  start_day = {start_day} !int
  start_hour = {start_hour} !double
  utc_start = 0 !double
...
!-----------------------------------------------------------------------
  ...
  dramp = 0.021 !needed if nramp=1; ramp-up period in days
...
!-----------------------------------------------------------------------
  ...
  iwbl = 2 !wave boundary layer formulation (used only if USE_WMM and 
           !icou_elfe_wwm/=0 and nchi=1. If icou_elfe_wwm=0, set iwbl=0): 
           !1-modified Grant-Madsen formulation; 2-Soulsby (1997)
  hmin_radstress = 0.1 !min. total water depth used only in radiation stress calculation [m]

  drampwafo = 0.125      !needed if nrampwafo=1; ramp-up period in days
  turbinj = 0.1      !% of depth-induced wave breaking energy injected in turbulence (default: 0.15 (15%), as proposed by Feddersen, 2012)
!  fwvor = 1              ! 1: on; 0: off --> wave-induced force (if RADFLAG='VOR')
...
!-----------------------------------------------------------------------
  nchi = 1
  dzb_min = 0.2 !needed if nchi=1; min. bottom boundary layer thickness [m].
...
!-----------------------------------------------------------------------
! Wetting and drying.
! - if ihhat=1, \hatH is made non-negative to enhance robustness near
...
!-----------------------------------------------------------------------
  ...
  shorewafo = 1 !Matters only if USE_WWM
...
!-----------------------------------------------------------------------
  nws = 2
  wtiminc = 3600. !Time interval for wind input in seconds (1 hour)
  iwind_form = -1 !Use Pond & Pichard formulation for wind stress
  drampwind = 0.04166666666 !Wind ramp-up period in days (6 hours)
...
!-----------------------------------------------------------------------
  ...
  xlsc0 = 1 !needed if itur=3 or 5. Scale for surface & bottom mixing length (>0)
...
!-----------------------------------------------------------------------
  ...
  iof_hydro(13) = 1 !Bottom drag coefficient [-]
  iof_hydro(14) = 1 !wind velocity vector [m/s]
  iof_hydro(15) = 1 !wind stress vector [m^2/s/s]
  iof_hydro(16) = 1 !depth-averaged vel vector [m/s]
  iof_hydro(17) = 1 !vertical velocity [m/s]
  iof_hydro(18) = 1 !water temperature [C]
  iof_hydro(19) = 1 !water salinity [PSU]
  iof_hydro(20) = 0 !water density [kg/m^3]
  iof_hydro(21) = 1 !eddy diffusivity [m^2/s]
  iof_hydro(22) = 1 !eddy viscosity [m^2/s]
...
!-----------------------------------------------------------------------
  iof_wwm(1)  = 1 !sig. height (m)
  iof_wwm(2)  = 1 !Mean average period (sec) - TM01
  iof_wwm(3)  = 1 !Zero down crossing period for comparison with buoy (s) - TM02
  iof_wwm(4)  = 1 !Average period of wave runup/overtopping - TM10
  iof_wwm(5)  = 1 !Mean wave number (1/m)
  iof_wwm(6)  = 1 !Mean wave length (m)
  iof_wwm(7)  = 1 !Mean average energy transport direction (degr) - MWD in NDBC?
  iof_wwm(8) = 1 !Mean directional spreading (degr)
  iof_wwm(9) = 1 !Discrete peak period (sec) - Tp
  iof_wwm(10) = 1 !Continuous peak period based on higher order moments (sec)
  iof_wwm(11) = 1 !Peak phase vel. (m/s)
  iof_wwm(12) = 1 !Peak n-factor
  iof_wwm(13) = 1 !Peak group vel. (m/s)
  iof_wwm(14) = 1 !Peak wave number
  iof_wwm(15) = 1 !Peak wave length
  iof_wwm(16) = 1 !Peak (dominant) direction (degr)
  iof_wwm(17) = 1 !Peak directional spreading
  iof_wwm(18) = 1 !Discrete peak direction (radian?)
  iof_wwm(19) = 1 !Orbital vel. (m/s)
  iof_wwm(20) = 1 !RMS Orbital vel. (m/s)
  iof_wwm(21) = 1 !Bottom excursion period (sec?)
  iof_wwm(22) = 1 !Bottom wave period (sec)
  iof_wwm(23) = 1 !Uresell number based on peak period
  iof_wwm(24) = 0 !Friction velocity (m/s?)
  iof_wwm(25) = 0 !Charnock coefficient
  iof_wwm(26) = 0 !Rougness length
  iof_wwm(27) = 0 !WWM_energy vector
  iof_wwm(28) = 0 !Wave force vector (m.s-2) computed by wwm @side centers and whole levels
...
def main():
    parser = argparse.ArgumentParser(description="Generate param.nml file for SCHISM")
    ...
    parser.add_argument("--hour", type=int, required=True, help="Start hour (can be fractional)")
    parser.add_argument("--rnday", type=int, required=True, help="Simulation periond")
    ...
    content = generate_param_nml(args.year, args.month, args.day, args.hour, args.rnday)
    ...
    outfile = outdir / "param.nml"

    with open(outfile, "w") as f:
        f.write(content)

    print("param.nml file has been generated successfully.")
