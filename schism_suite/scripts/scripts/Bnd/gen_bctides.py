...
class CustomBctides(Bctides):
    def write(self, path, start_date, rnday, overwrite=False):
        output_file = pathlib.Path(path) / 'bctides.in'
        with output_file.open('w') as f:
            f.write(f"{start_date.strftime('%m/%d/%Y %H:%M:%S')} UTC\n")
            f.write(f" 0  {self.cutoff_depth:.3f}   ! number of earth tidal potential, cut-off depth for applying tidal potential\n")
            f.write(" 0          ! number of boundary forcing freqs\n")
            f.write(f" {len(self.flags)}          ! number of open boundaries\n")
            
            for i, flag in enumerate(self.flags):
                boundary = self.hgrid.boundaries.open.iloc[i]
                f.write(f" {len(boundary.indexes)} {flag[0]} {flag[1]} {flag[2]} {flag[3]} 0 ! type of b.c.\n")
...
if __name__ == "__main__":
    ...
    for ibnd, flag in enumerate(flags):
        ...
        # Navigate up two levels and then to 'workdir'
        outdir = script_dir.parent.parent / 'workdir'
